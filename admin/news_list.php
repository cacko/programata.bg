...
$aSectionPages = $oPage->ListAllParentsAsArraySimple(null, '', 'news_list', true);
...
<tr>
	...
	<td><label for="city"><?=getLabel('strCity')?></label><br />
	<select name="city" id="city" class="fldfilter">
		<option value=""><?=getLabel('strAny')?></option>
	<?
		foreach($aCities as $key=>$val)
		{
			echo '<option value="'.$key.'"';
			if ($key == getRequestArg('city', $nDefCity))
				echo ' selected="selected"';
			echo '>'.$val.'</option>'."\n";
		}
	?>
...
//=========================================================
	...
	$rs = $oNews->ListAll(getRequestArg('parent_page',null), getRequestArg('city',$nDefCity), getRequestArg('keyword'),
...
<thead>
<tr>
	<td><?=getLabel('strNewsImage')?></td>
	<td><a href="<?=setPageContext(setContext(1,$aContext[CUR_REC])).setFilterParam($aFilter)?>"><?=getLabel('strNewsID')?></a></td>
	<td><a href="<?=setPageContext(setContext(2,$aContext[CUR_REC])).setFilterParam($aFilter)?>"><?=getLabel('strNewsDate')?></a></td>
	<td><a href="<?=setPageContext(setContext(3,$aContext[CUR_REC])).setFilterParam($aFilter)?>"><?=getLabel('strNewsName')?></a> (<a href="<?=setPageContext(setContext(10,$aContext[CUR_REC])).setFilterParam($aFilter)?>"><?=getLabel('strNrViews')?></a>)</td>
...
<?
		...
		while ($row = mysql_fetch_object($rs))
		{
			if ($curRow >= $nStartRow && $curRow < $nEndRow)
			{
				...
				$sCities = '';
				$aRelCities = $oNews->ListNewsCitiesAsArray($row->NewsID);
...
<tr class="<?=IIF($curRow%2==0, 'odd', 'even').IIF($row->IsHidden == true, ' hidden', '')?>" onmouseover="lt(this,1)" onmouseout="lt(this,0)">
	...
	<td><?=$row->NewsID?></td>
	<td><?=formatDate($row->NewsDate)?></td>
	<td><strong><?=$row->Title?></strong> (<?=$row->NrViews?>)</td>
	<td><?=$sPages?></td>
	<td><?=$sCities?></td>
	<!--td><?=$row->LastUpdate?><br /><?=getLabel('strByUser').$aUsers[$row->LastUpdateUserID]?></td-->
	<td><a href="<?=setPage($page, 0, $row->NewsID, ACT_VIEW).keepFilter().keepContext()?>"><?=getLabel('view')?></a> | <a href="<?=setPage($page, 0, $row->NewsID, ACT_EDIT).keepFilter().keepContext()?>"><?=getLabel('edit')?></a> | <a href="<?=setPage($page, 0, $row->NewsID, ACT_DELETE).keepFilter().keepContext()?>" onclick="return confMsg('<?=getLabel('strDeleteQ')?>');return false;"><?=getLabel('delete')?></a></td>
</tr>
<?
			}
			$curRow++;
