...
$aPagesSimple = $oPage->ListAllAsArraySimple(null, '', 'urban_list', true);
//=========================================================
...
//=========================================================
	...
	$rs = $oUrban->ListAll(getRequestArg('parent_page',null), getRequestArg('keyword'),
...
<thead>
<tr>
	<td><?=getLabel('strUrbanImage')?></td>
	<td><a href="<?=setPageContext(setContext(1,$aContext[CUR_REC])).setFilterParam($aFilter)?>"><?=getLabel('strUrbanID')?></a></td>
	<td><a href="<?=setPageContext(setContext(2,$aContext[CUR_REC])).setFilterParam($aFilter)?>"><?=getLabel('strPublicationDate')?></a></td>
	<td><a href="<?=setPageContext(setContext(3,$aContext[CUR_REC])).setFilterParam($aFilter)?>"><?=getLabel('strUrbanName')?></a> (<a href="<?=setPageContext(setContext(10,$aContext[CUR_REC])).setFilterParam($aFilter)?>"><?=getLabel('strNrViews')?></a>)</td>
...
<?
		...
		while ($row = mysql_fetch_object($rs))
		{
			...
			{
				$sPages = '';
				$aRelPages = $oUrban->ListUrbanPagesAsArray($row->UrbanID);
				...
				$sMainImageFile = '../'.UPLOAD_DIR.IMG_URBAN.$row->UrbanID.'/'.IMG_THUMB.'1_1.'.EXT_IMG;
...
<tr class="<?=IIF($curRow%2==0, 'odd', 'even').IIF($row->IsHidden == true, ' hidden', '')?>" onmouseover="lt(this,1)" onmouseout="lt(this,0)">
	...
	<td><?=$row->MainTitle?> (<?=$row->NrViews?>)</td>
	<td><?=$sPages?></td>
	<!--td><?=$row->LastUpdate?><br /><?=getLabel('strByUser').$aUsers[$row->LastUpdateUserID]?></td-->
	<td><a href="<?=setPage($page, 0, $row->UrbanID, ACT_VIEW).keepFilter().keepContext()?>"><?=getLabel('view')?></a> | <a href="<?=setPage($page, 0, $row->UrbanID, ACT_EDIT).keepFilter().keepContext()?>"><?=getLabel('edit')?></a> | <a href="<?=setPage($page, 0, $row->UrbanID, ACT_DELETE).keepFilter().keepContext()?>" onclick="return confMsg('<?=getLabel('strDeleteQ')?>');return false;"><?=getLabel('delete')?></a></td>
</tr>
<?
			}
			$curRow++;
