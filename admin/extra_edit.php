...
switch($action)
{
	case ACT_SAVE:
		...
		{
			$item = $oExtra->Insert(	getPostedArg('title'.$aAbbrLanguages[DEF_LANG]), 
		...
		// save relations
		$oExtra->DeleteExtraPage($item);
		$oExtra->InsertExtraPage($item, getPostedArg('parent_page_id'));
		// UPLOAD FILE HERE //==========================================
		$nError = uploadBrowsedFile('main_image', IMG_EXTRA.$item, EXT_IMG);/*, 'empty.jpg'*/
		if (empty($nError))
		{
			resizeImage(IMG_EXTRA.$item.'.'.EXT_IMG, W_IMG_GALLERY, H_IMG_GALLERY);
			deleteFile(IMG_EXTRA_MID.$item, EXT_IMG);
			duplicateFile(IMG_EXTRA.$item.'.'.EXT_IMG, IMG_EXTRA_MID.$item.'.'.EXT_IMG);
			resizeImage(IMG_EXTRA_MID.$item.'.'.EXT_IMG, W_IMG_MIDDLE, H_IMG_MIDDLE);
			deleteFile(IMG_EXTRA_THUMB.$item, EXT_IMG);
			duplicateFile(IMG_EXTRA.$item.'.'.EXT_IMG, IMG_EXTRA_THUMB.$item.'.'.EXT_IMG);
			resizeImage(IMG_EXTRA_THUMB.$item.'.'.EXT_IMG, W_IMG_THUMB, H_IMG_THUMB);
		}
		...
		$aRows = array();
		foreach($aLanguages as $key=>$val)
			$aRows[$key] = $oExtra->GetByID($item, $key); // get by id
	...
	case ACT_DELETE:
		...
		{
			// DELETE FILE HERE //==========================================
			deleteFile(IMG_EXTRA.$item, EXT_IMG);
			deleteFile(IMG_EXTRA_MID.$item, EXT_IMG);
			deleteFile(IMG_EXTRA_THUMB.$item, EXT_IMG);
			// DELETE FILE HERE //==========================================
			$oExtra->Delete($item);
...
{
	global $page, $oPage, $oExtra, $oUser, $aLanguages, $aAbbrLanguages;
...
<fieldset title="<?=getLabel('strCommonData')?>" class="half">
	...
	<?
		...
		{
			$aRelPages = $oExtra->ListExtraPagesAsArray($itemID);
		}
...
<fieldset title="<?=getLabel('strCommonData')?>" class="half">
	...
	<? 	if (!empty($aRows[DEF_LANG]) )
		{
			$sMainImageFile = '../'.UPLOAD_DIR.IMG_EXTRA_THUMB.$itemID.'.'.EXT_IMG;
...
{
	global $page, $oPage, $oExtra, $oUser, $aLanguages, $aEntityTypes;
...
<iframe name="related" style="float:left;" width="500" height="600" src="<?=RELATED_PAGE.'?'.ARG_PAGE.'='.ENT_HOME.'&amp;'.ARG_RELID.'='.$itemID.'&amp;'.ARG_CAT.'='.$aEntityTypes[ENT_EXTRA]?>"></iframe>
<br class="clear" />
<?
	foreach($aLanguages as $key=>$val)
	{
