...
class Address
{
	...
	function Address($dbCon, $nLangID)
	{
		$this->_sTable = DB_TABLE_PREFIX.'tblAddress';
		$this->_dbCon = $dbCon;
		$this->_nLangID = $nLangID;

		$this->_aFields = array();

		$this->_aFields[1] = 'AddressID';
		$this->_aFields[2] = 'AddressTypeID';
		$this->_aFields[3] = 'CityID';
		$this->_aFields[4] = 'Street';
		$this->_aFields[5] = 'Zip';
	...
	function ListAll($nEntityID=null, $nEntityTypeID=null, $xAddressTypeID=null, $bShowHidden = false, $aOrder=null)
	{
		...
		if (!is_null($xAddressTypeID))
		{
			if (is_array($xAddressTypeID) && count($xAddressTypeID) > 0)
				$sSQL .= ' AND AddressTypeID IN ('.join(',', $xAddressTypeID).')';
			elseif (is_numeric($xAddressTypeID) && !empty($xAddressTypeID))
				$sSQL .= ' AND AddressTypeID="'.$xAddressTypeID.'" ';
		}
		if (!$bShowHidden)
		{
	...
	function ListAllAsArray($nEntityID=null, $nEntityTypeID=null, $nAddressTypeID=null, $bShowHidden = false)
	{
		$result = $this->ListAll($nEntityID, $nEntityTypeID, $nAddressTypeID, $bShowHidden);
		$aToReturn = array();
		while($row = mysql_fetch_object($result))
		{
			$aToReturn[$row->AddressID] = $row->Title;
		}
	...
	function Insert($nAddressTypeID, $nCityID, $sStreet, $sZip, $nEntityID, $nEntityTypeID, $bIsHidden=0)
	{
	...
	function Update($nID, $nAddressTypeID, $nCityID, $sStreet, $sZip, $nEntityID, $nEntityTypeID, $bIsHidden=0, $nLangID=0)
	{
		global $oSession;
