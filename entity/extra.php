...
class Extra
{
	...
	function Extra($dbCon, $nLangID)
	{
		$this->_sTable = DB_TABLE_PREFIX.'tblExtra';
		$this->_sRelPageTable = DB_TABLE_PREFIX.'relExtra2Page';
		$this->_dbCon = $dbCon;
	...
	function Delete($nID)
	{
		...
		$this->DeleteExtraPage($nID);
		
		return true;
	}
	
	function InsertExtraPage($nPublicationID, $xPageID)
	{
		foreach(func_get_args() as $xParam) stripParam($xParam);
	...
	function DeleteExtraPage($nPublicationID=0, $nPageID=0)
	{
		foreach(func_get_args() as $xParam) stripParam($xParam);
	...
	function ListExtraPagesAsArray($nPublicationID)
	{
		foreach(func_get_args() as $xParam) stripParam($xParam);
